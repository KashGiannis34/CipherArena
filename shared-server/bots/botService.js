...
function getShell() {
  ...
  initPromise = new Promise((resolve, reject) => {
    const py = new PythonShell('codebusters_bot_server.py', {
      mode: 'text',
      pythonPath:
        process.env.PYTHON_PATH || (process.env.NODE_ENV === 'production' ? 'python3' : 'python'),
      pythonOptions: ['-u', '-O'],
...
// Public API
export const generateProblem = (problemType, decimals) =>
  send('generate', { problemType, decimals });
export const checkAnswer = (problemType, problemData, userAnswer) =>
  send('check', { problemType, problemData, userAnswer });
