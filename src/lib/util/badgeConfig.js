...
},
{
  ...
  progress: stats => {
    ...
    if (times.some(t => t < 10)) return undefined;
...
},
{
  ...
  progress: stats => {
    const count = stats?.All?.solveTimes?.filter(t => t < 30).length ?? 0;
...
},
{
  ...
  progress: stats => {
    const count = stats?.All?.solveTimes?.filter(t => t < 60).length ?? 0;
...
},
{
  ...
  progress: stats => {
    const count = stats?.Aristocrat?.solveTimes?.filter(t => t < 34).length ?? 0;
...
},
{
  ...
  progress: stats => {
    const times = stats?.All?.solveTimes ?? [];
    if (times.includes(59)) return undefined;
    if (times.length === 0) return 'No solves yet';
    const closest = times.reduce((prev, curr) =>
      Math.abs(curr - 59) < Math.abs(prev - 59) ? curr : prev, times[0]
    );
    return `${closest}s (need exactly 59s)`;
  }
},
{
  id: 'slow_grinder',
  ...
  progress: stats => {
    const times = stats?.All?.solveTimes ?? [];
    if (times.some(t => t >= 300)) return undefined;
    if (times.length === 0) return 'No solves yet';
...
},
{
  ...
  progress: stats => {
    ...
    const timeSet = new Set(times.map(t => Math.round(t)));
