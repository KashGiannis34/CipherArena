  ...
  async function fetchLeaderboard(cipherType, metric) {
    try {
      const res = await fetch(
        `/api/leaderboard?cipherType=${cipherType}&metric=${metric === 'win%' ? 'winPercent' : metric}&count=${count}`
      );
  ...
  {#if !simple}
    <div class="metric-toggle">
      {#each metricOptions as m}
        <button class:selected={metric === m} onclick={() => handleMetricSelect(m)}>
          {m}
        </button>
      {/each}
    </div>
...
<style>
  ...
  @keyframes fadeGlowIn {
    ...
    to {
      box-shadow:
        0 0 8px var(--color-pink-glow),
        inset 0 0 2px var(--color-pink-glow);
    }
  }
  ...
  @keyframes rainbowGlow {
    ...
    25% {
      box-shadow:
        0 0 8px var(--color-link),
        inset 0 0 2px var(--color-link);
    }
    50% {
      box-shadow:
        0 0 8px var(--color-green-glow),
        inset 0 0 2px var(--color-green-glow);
    }
    75% {
      box-shadow:
        0 0 8px var(--color-warning),
        inset 0 0 2px var(--color-warning);
    }
    100% {
      box-shadow:
        0 0 8px var(--color-pink-glow),
