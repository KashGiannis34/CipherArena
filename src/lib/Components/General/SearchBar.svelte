  ...
  function updateAutocomplete() {
    if (!currentInput) {
      if (activeKey) {
        ...
        const hasOtherKeys = value
          .split(' ')
          .some((word) => isCompleteToken(word) && word !== 'all');
  ...
  function handleKeydown(e) {
    if (showAutocomplete && autocompleteOptions.length > 0) {
      if (e.key === 'ArrowDown') {
        ...
        e.preventDefault();
        selectedOptionIndex =
          (selectedOptionIndex - 1 + autocompleteOptions.length) % autocompleteOptions.length;
  ...
  <div class="search-input-wrapper">
    {#each tokens as token, i}
      {#if token.isToken}
        {#if token.type === 'key'}
          <span
            class="search-token {token.text === 'all' ? 'all-token' : ''} {tokens[i + 1]?.type ===
            'value'
              ? 'has-value'
              : ''}">{token.text}</span
          >
    ...
    {/each}
    {#if activeKey}
      <span class="search-token active-key {activeKey === 'all' ? 'all-token' : ''}"
        >{activeKey}</span
      >
    {/if}
    <input
    ...
    <div class="autocomplete" transition:fade={{ duration: 100 }}>
      ...
      {#each autocompleteOptions as option, i}
        <div
          ...
          aria-label={option.type === 'full-pair'
            ? option.label
