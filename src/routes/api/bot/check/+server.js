...
import { decryptToken } from "$server/utils/textEncode";
...
export async function POST({ request }) {
  try {
    ...
    let decryptedData;
    try {
      const decryptedString = decryptToken(encryptedAnswer);
      if (!decryptedString) {
    ...
    return json({
      ...
      ...(result.correct
        ...
        : {
            ...
            ...(result.correct_answer_mod_26 !== undefined && {
              correctAnswerMod26: result.correct_answer_mod_26,
            }),
            ...(result.correct_answer_letter !== undefined && {
              correctAnswerLetter: result.correct_answer_letter,
            }),
          }),
    });
  } catch (error) {
