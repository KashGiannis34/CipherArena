...
export async function POST({ request, cookies }) {
  try {
    ...
    const [userGame, userAuth] = await Promise.all([
      UserGame.findById(userId).select('_id currentGame username'),
    ...
    if (userGame.currentGame)
      return json({
        success: false,
        message:
          'You are already in a game. Leave the current game before creating or joining another.',
    ...
    if (userAuth.verified == false)
      return json({
        success: false,
        message: 'Must verify email to create games.',
      });
    ...
    if (isNaN(playerLimit) || playerLimit < 2 || playerLimit > 6) {
      return json({
        success: false,
        message: 'Invalid player limit. Must be between 2 and 6.',
      });
    }
