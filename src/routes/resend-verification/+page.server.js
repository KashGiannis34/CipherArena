  ...
  try {
    ...
    if (user.lastVerificationRequest) {
      ...
      if (timeDiff < RATE_LIMIT_MINUTES) {
        return {
          message: `In order to prevent spam-sending verification emails, we have not sent another email. Please wait ${Math.ceil(RATE_LIMIT_MINUTES - timeDiff)} ${Math.ceil(RATE_LIMIT_MINUTES - timeDiff) == 1 ? 'minute' : 'minutes'} before trying again.`,
          sent: false,
        };
      }
    }
    ...
    return {
      message: 'Verification email resent to ' + user.email + '. Check your inbox and spam folder.',
      sent: true,
    };
  } catch (error) {
