  ...
  io.on('connection', async (socket) => {
      ...
      try {
        ...
        if (user.currentGame) {
          game = await Game.findById(user.currentGame)
            .populate({
              path: 'users',
              select: 'username profilePicture stats currentSocketId _id',
            })
            .exec();
          if (!game) {
    ...
    if (socket.currentRoom != 'public-lobby') {
      ...
      socket.on('join-room', async () => {
        try {
          const hostUG = await UserGame.findById(game.host).select('_id currentSocketId').lean();
          ...
          socket.emit('error', 'Internal error during join room.');
        }
      });
        ...
        try {
          await withGameLock(user.currentGame, async () => {
            game = await Game.findById(user.currentGame)
              .populate({
                path: 'users',
                select: 'username profilePicture _id currentSocketId',
              })
              .exec();
            ...
            const targetSocket = io.sockets.sockets.get(
              activeSockets.get(targetUser._id.toString())
            );
            if (targetSocket) {
        ...
        try {
          await withGameLock(user.currentGame, async () => {
            game = await Game.findById(user.currentGame)
              .populate({
                path: 'users',
                select: 'username profilePicture stats _id currentSocketId',
              })
              .exec();
            ...
            if (
              game.mode === 'ranked' &&
              game.users.length === 1 &&
              user._id.equals(game.users[0]._id)
            ) {
      ...
      socket.on('get-cipher-info', async (cb) => {
        try {
          ...
          if (!game.state == 'started') {
            ...
            cb({
              params: game.params,
              autoFocus: game.autoFocus,
              quote: game.quote,
            });
          }
        } catch (err) {
          socket.emit('error', 'Internal error during info retrieval.');
        }
      });
      ...
      socket.on('get-initial-players', async (cb) => {
        try {
          ...
          const initialPlayers = await UserGame.find({
            _id: { $in: game.metadata.initialUserIds },
          })
            .select('username profilePicture stats _id currentSocketId')
            .lean();
      ...
      socket.on('check-quote', async (ans, quoteId, cipherType, keys, solve, cb) => {
        try {
          game = await Game.findById(user.currentGame)
            .populate({
              path: 'users',
              select: 'username _id currentSocketId',
            })
            .exec();
          const res = await wsUtil.checkAnswerCorrectness(
            ans,
            quoteId,
            game.quote?.id,
            cipherType,
          ...
          await withGameLock(user.currentGame, async () => {
            ...
            if (game.state != 'started') {
              socket.emit(
                'error',
                'The game is not in the game started state, please refresh the page'
              );
              return;
            }
            ...
            if (game.mode === 'ranked' && game.metadata?.initialUserIds?.length > 1) {
              const initialPlayers = await UserGame.find({
                ...
                .select('username stats _id')
                .lean();
              eloChanges = await wsUtil.updateStatsAfterWin(
                redis,
                initialPlayers,
                user,
                cipherType,
            ...
            const initialPlayers = await UserGame.find({
              ...
              .select('username stats profilePicture _id')
              .lean();

            if (
              game.mode === 'ranked' &&
              game.metadata?.initialUserIds?.length === 1 &&
              game.metadata.initialUserIds[0].equals(user._id)
            ) {
      ...
      socket.on('forfeit-request', async (shouldForfeit) => {
        ...
        await withGameLock(user.currentGame, async () => {
          game = await Game.findById(user.currentGame)
            .populate({ path: 'users', select: 'username _id currentSocketId' })
            .exec();
          if (!game) return;
      ...
      socket.on('progress-update', ({ username, progress }) => {
        io.to(socket.currentRoom).emit('progress-map-update', {
          username,
          progress,
        });
      });
      ...
      socket.on('disconnect', async () => {
        try {
          ...
          const latestUser = await UserGame.findById(userId).lean();
          ...
          if (!socket.disconnectReason || socket.disconnectReason == 'leftGame') {
            if (latestUser.currentGame) {
              latestGame = await Game.findById(latestUser.currentGame)
                .populate({
                  path: 'users',
                  select: 'username profilePicture stats currentSocketId _id',
                })
            ...
            if (
              latestGame &&
              latestGame.host.equals(latestUser._id) &&
              (latestGame.state !== 'waiting' || socket.disconnectReason === 'leftGame')
            ) {
              const newHost = latestGame.users.find((u) => u.currentSocketId != null);
              if (newHost) {
                await Game.findByIdAndUpdate(latestGame._id, {
                  host: newHost._id,
                });
              }
            }
          ...
          socket.emit('error', 'Internal error during disconnect.');
        }
      });
    } else {
      io.to(socketId).emit('ready');
            ...
            games = games.filter((game) => {
              for (const [option, terms] of Object.entries(searchTerms)) {
                ...
                switch (option) {
                  ...
                  case 'username:':
                    if (
                      !game.users.some((user) => user.username?.toLowerCase().includes(searchText))
                    ) {
                      return false;
                    }
                    break;
...
async function handleForfeitRequest(
  game,
  shouldForfeit,
  io,
  forfeitVotesMap,
  ...
  const connectedUsernames = game.users.filter((u) => u.currentSocketId).map((u) => u.username);
